...
func (c *Cursor) fetchRows() error {
	...
	c.rows = make([]EventModel, 0, c.limit)
	for rows.Next() {
		var item EventModel
		err := rows.Scan(
			&item.ID,
...
func (c *Cursor) Current() (EventModel, error) {
	if c.closed {
		return EventModel{}, errors.New("cursor is closed")
	}

	if c.index >= 0 && c.index < len(c.rows) {
		return c.rows[c.index], nil
	}
	return EventModel{}, errors.New("not found")
}
